...
(function() {
    ...
    compile = function() {
        ...
        var MyLanguageServiceHost = (function () {
            ...
            MyLanguageServiceHost.prototype.trace = function (msg) {
                console.log('trace', msg);
            };
            MyLanguageServiceHost.prototype.error = function (msg) {
                console.log('error', msg);
            };
            MyLanguageServiceHost.prototype.getScriptVersion = function (fileName) {
                if (!this.files[fileName])
                    return null;
                return this.files[fileName].ver.toString();
            };
            MyLanguageServiceHost.prototype.getScriptSnapshot = function (fileName) {
        ...
        for(i = 0; i < script.length; i++) {
            if(script[i].type == 'text/typescript') {
                if(script[i].src) {
                    src.push({
                        filename: script[i].getAttribute("src"),
                        data: load(script[i].src)
                    });
                } else {
                    src.push({
                        filename: 'inline.' + i + '.ts',
                        data: script[i].innerHTML
                    });
                }
            }
        }
        ...
        host.addFile("lib.d.ts", libfile);

        var file;
        for(file = 0; file < src.length; file++) {
            if (src[file].filename !== null) {
                host.addFile(src[file].filename, src[file].data);
            }
        }

        for(file = 0; file < src.length; file++) {
            if (src[file].filename !== null) {
                var currentOutput = languageService.getEmitOutput(src[file].filename);
                var errors = logErrors(src[file].filename);
                if (errors.length > 0) {
                    allErrors[src[file].filename] = errors;
                }
                src[file].compiled = currentOutput.outputFiles[0].text;
